...
struct packet p_init(unsigned seq_number, const char *arg, enum type msg_type, unsigned long token)
{
    struct packet p = {.seq_number = seq_number, .arg = malloc(strlen(arg) + 1),
                        .msg_len = strlen(arg), .msg_type = msg_type, .error = 0, .token = token};
    strcpy(p.arg, arg);
    return p;
}
